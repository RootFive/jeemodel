...
CREATE TABLE `gen_table`  (
  ...
  `vue_version` 				varchar(4)  		NOT NULL DEFAULT 'Vue3'					COMMENT 'VUE版本（V2和Vue3）',
...
CREATE TABLE `gen_table_column`  (
  ...
  `is_required` 		tinyint(1)  		NOT NULL DEFAULT '0' 			COMMENT '是否必填（1是）',
  `is_insert` 			tinyint(1)  		NOT NULL DEFAULT '1' 			COMMENT '是否为插入字段（1是）',
  `is_edit` 			tinyint(1)  		NOT NULL DEFAULT '0' 			COMMENT '是否编辑字段（1是）',
  `is_list` 			tinyint(1)  		NOT NULL DEFAULT '0' 			COMMENT '是否列表字段（1是）',
  `is_query` 			tinyint(1)  		NOT NULL DEFAULT '0' 			COMMENT '是否查询字段（1是）',
