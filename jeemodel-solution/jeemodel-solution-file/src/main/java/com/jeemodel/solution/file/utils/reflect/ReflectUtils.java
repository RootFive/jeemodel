package com.jeemodel.solution.file.utils.reflect;
			...
			for (int i = 0; i < cs.length; i++) {
				if (args[i] != null && !args[i].getClass().equals(cs[i])) {
					if (cs[i] == String.class) {
						args[i] = DataTypeConvertUtils.toStr(args[i]);
						if (StringUtils.endsWith((String) args[i], ".0")) {
							args[i] = StringUtils.substringBefore((String) args[i], ".0");
						}
					} else if (cs[i] == Integer.class) {
						args[i] = DataTypeConvertUtils.toInt(args[i]);
					} else if (cs[i] == Long.class) {
						args[i] = DataTypeConvertUtils.toLong(args[i]);
					} else if (cs[i] == Double.class) {
						args[i] = DataTypeConvertUtils.toDouble(args[i]);
					} else if (cs[i] == Float.class) {
						args[i] = DataTypeConvertUtils.toFloat(args[i]);
						...
						args[i] = DataTypeConvertUtils.toBool(args[i]);
					}
				}
			}
			return (E) method.invoke(obj, args);
